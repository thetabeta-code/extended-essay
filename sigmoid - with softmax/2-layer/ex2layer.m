function [] = ex2layer(trainX, trainY, testX, testY,...
...
cd "D:/Oakridge/DP-1/Extended_essay/Dataset/sigmoid - with softmax/2-layer";
...
predTest = predict2layer(Theta1, Theta2, Theta3, testX);

test_accuracy = mean(double(predTest == testY')) * 100;

fprintf('\nTest Set Accuracy: %f\n', test_accuracy);

csvwrite(strcat('sigmoid_L2N', int2str(hidden_layer_size), 'I', int2str(maxIter), '_R', int2str(iteration), '.dat'), newcostvals);
csvwrite(strcat('sigmoid_L2N', int2str(hidden_layer_size), 'I', int2str(maxIter), '_R', int2str(iteration), '_acc.dat'), final_accuracy);
csvwrite(strcat('sigmoid_L2N', int2str(hidden_layer_size), 'I', int2str(maxIter), '_R', int2str(iteration), '_testacc.dat'), test_accuracy);

save(strcat('sigmoid_L2N', int2str(hidden_layer_size), 'I', int2str(maxIter), '_R', int2str(iteration), 'Theta1.mat'), 'Theta1');
save(strcat('sigmoid_L2N', int2str(hidden_layer_size), 'I', int2str(maxIter), '_R', int2str(iteration), 'Theta2.mat'), 'Theta2');
save(strcat('sigmoid_L2N', int2str(hidden_layer_size), 'I', int2str(maxIter), '_R', int2str(iteration), 'Theta3.mat'), 'Theta3');
