function [] = ex4layer(trainX, trainY, testX, testY, ...
...
hidden_layer_4_size = hidden_layer_size;
num_labels = 10;

cd "D:/Oakridge/DP-1/Extended_essay/Dataset/sigmoid - with softmax/4-layer";
...
initial_Theta4 = randInitializeWeights(hidden_layer_3_size, hidden_layer_4_size);
initial_Theta5 = randInitializeWeights(hidden_layer_4_size, num_labels);
...
costFunction = @(p) costFunction4layer(p, ...
                                       input_layer_size, ...
                                       hidden_layer_1_size, ...
                                       hidden_layer_2_size, ...
                                       hidden_layer_3_size, ...
...
first_cost = costFunction4layer(initial_nn_params,...
                                input_layer_size,...
                                hidden_layer_1_size,...
                                hidden_layer_2_size,...
                                hidden_layer_3_size,...
...
pred = predict4layer(Theta1, Theta2, Theta3, Theta4, Theta5, trainX);

fprintf('\nTraining Set Accuracy: %f\n', mean(double(pred == trainY')) * 100);

final_accuracy = mean(double(pred == trainY')) * 100;

predTest = predict4layer(Theta1, Theta2, Theta3, Theta4, Theta5, testX);

test_accuracy = mean(double(predTest == testY')) * 100;

fprintf('\nTest Set Accuracy: %f\n', test_accuracy);

csvwrite(strcat('sigmoid_L4N', int2str(hidden_layer_size), 'I', int2str(maxIter), '_R', int2str(iteration), '.dat'), newcostvals);
csvwrite(strcat('sigmoid_L4N', int2str(hidden_layer_size), 'I', int2str(maxIter), '_R', int2str(iteration), '_acc.dat'), final_accuracy);
csvwrite(strcat('sigmoid_L4N', int2str(hidden_layer_size), 'I', int2str(maxIter), '_R', int2str(iteration), '_testacc.dat'), test_accuracy);

save(strcat('sigmoid_L4N', int2str(hidden_layer_size), 'I', int2str(maxIter), '_R', int2str(iteration), 'Theta1.mat'), 'Theta1');
save(strcat('sigmoid_L4N', int2str(hidden_layer_size), 'I', int2str(maxIter), '_R', int2str(iteration), 'Theta2.mat'), 'Theta2');
save(strcat('sigmoid_L4N', int2str(hidden_layer_size), 'I', int2str(maxIter), '_R', int2str(iteration), 'Theta3.mat'), 'Theta3');
save(strcat('sigmoid_L4N', int2str(hidden_layer_size), 'I', int2str(maxIter), '_R', int2str(iteration), 'Theta4.mat'), 'Theta4');
save(strcat('sigmoid_L4N', int2str(hidden_layer_size), 'I', int2str(maxIter), '_R', int2str(iteration), 'Theta5.mat'), 'Theta5');

cd "D:/Oakridge/DP-1/Extended_essay/Dataset/sigmoid - with softmax";
end